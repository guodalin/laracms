...
class ArticleController extends Controller
{
    ...
    public function index($navigation = 0, Category $articleCategory, Article $article)
    {
        ...
        // dd($articleCategory,$children);
        return frontend_view('article.'.$articleCategory->getTemplate('list'), compact('navigation','category','articles', 'children'));
    }
     ...
     * @return mixed
     */
    public function child($navigation = 0, Category $articleCategory, Article $article)
    {
        $category = $articleCategory;
        $articles = $category->articles()->active()->ordered()->recent()->paginate(10);
        $children = $category->brother();
        // dd($articleCategory,$children);
        return frontend_view('article.' . $articleCategory->getTemplate('list'), compact('navigation', 'category', 'articles', 'children', 'articleCategory'));
    }
